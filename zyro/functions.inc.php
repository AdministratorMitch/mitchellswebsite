...
function parse_uri(SiteInfo $siteInfo) {
	$nh = preg_replace('#/\./#', '/', $siteInfo->baseUrl);
...
function handleForms($page_id, SiteInfo $siteInfo) {
	...
	try {
		global $wb_form_send_state, $wb_form_id, $formErrors;
		...
		foreach($fields as $idx => $field) {
			...
			if( $field["type"] === "file" ) {
				...
				if (!isset($post[$fieldName])) {
					...
					throw new ErrorException($wb_form_sending_failed.' (6)');
				}
				...
				if( $field["type"] === "file" ) {
					...
					if( !file_exists($attachmentsDir) ) {
						if( !mkdir($attachmentsDir, 0700) ) {
							...
							throw new ErrorException($wb_form_sending_failed.' (1)');
						}
					}
					if( !is_dir($attachmentsDir) ) {
						error_log('[Form error]: Attachments inode on the server is not a directory');
						throw new ErrorException($wb_form_sending_failed.' (2)');
					}
					foreach( $_FILES[$fieldName]["tmp_name"] as $fileIdx => $fileTmpName) {
						if( !$fileTmpName )
							continue;
						...
						if( !move_uploaded_file($fileTmpName, $tmpCopyName) ) {
							...
							throw new ErrorException($wb_form_sending_failed.' (3)');
						}
	...
	<html>
		...
		<body>' . $message . '</body>
			...
			if ($res) {
				...
				throw new ErrorException($wb_form_sending_failed.' (4)');
			}
			...
			throw new ErrorException($wb_form_sending_failed.' (5)');
		}
